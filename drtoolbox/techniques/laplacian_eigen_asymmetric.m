...
% X = X';
    ...
    % Construct eigenmaps (solve Ly = lambda*y)
    disp('Constructing Eigenmaps...');
	tol = 0;
    if strcmp(eig_impl, 'JDQR')
        options.Disp = 0;
        options.LSolver = 'bicgstab';
        [mappedX, lambda] = jdqz(L, eye(size(G, 1)), no_dims + 1, tol, options);			% only need bottom (no_dims + 1) eigenvectors
    else
        options.disp = 0;
        options.isreal = 1;
        options.issym = 1;
        [mappedX, lambda] = eigs(L, eye(size(G, 1)), no_dims + 1, tol, options);			% only need bottom (no_dims + 1) eigenvectors
    end
