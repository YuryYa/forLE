...
%     end
    ...
    switch type
        ...
        case {'LaplacianEigenmapsAsymmetric'}
            % Compute Laplacian Eigenmaps-based mapping
            if isempty(varargin) 
              [mappedA, mapping] = laplacian_eigen_asymmetric(A, no_dims, 12, 1, eig_impl);
			      elseif length(varargin) == 1 
              [mappedA, mapping] = laplacian_eigen_asymmetric(A, no_dims, varargin{1}, 1, eig_impl);
            elseif length(varargin) > 1
              [mappedA, mapping] = laplacian_eigen_asymmetric(A, no_dims, varargin{1}, varargin{2}, eig_impl); 
            end
            mapping.name = 'LaplacianEigenmapsAsymmetric';
